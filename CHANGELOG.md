...
- `Engineering/Docs/00-README.md` — a documentation index and reading map, so the docs are discoverable instead of scattered (the `00-` prefix floats it to the top of a directory listing). Now also states the organizing principle: the docs are a **knowledge tree** with four teaching modes — Explain (reference), Guide (tutorial/task-guide), Do (lesson), Judge (case study) — and a new doc is placed by choosing its mode.
