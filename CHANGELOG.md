...
- `Engineering/Docs/README.md` — a documentation index and reading map, so the docs are discoverable instead of scattered. Now also states the organizing principle: the docs are a **knowledge tree** with four teaching modes — Explain (reference), Guide (tutorial/task-guide), Do (lesson), Judge (case study) — and a new doc is placed by choosing its mode.
